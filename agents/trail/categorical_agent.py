"""CategoricalAgent — analysis for categorical columns."""
...
def _build_summary(col: str, table: str, a: dict) -> str:
    ...
    seg_note = ""
    if seg:
        top_seg = sorted(seg.items(), key=lambda x: x[1] if isinstance(x[1], (int, float)) else 0, reverse=True)[:3]
        seg_note = f"\nSegmentation top: {top_seg}"
    return (
        f"Column '{col}' | Table '{table}' | {freq.get('total_rows', 0)} rows\n"
        f"Cardinality: {card.get('unique_count')} unique ({card.get('type')}), "
        f"ratio={card.get('cardinality_ratio')}\n"
        f"Dominant: '{freq.get('dominant_category')}' at {freq.get('dominant_pct')}%, "
        f"rare categories: {freq.get('rare_category_count')}\n"
        f"Imbalanced: {imb.get('is_imbalanced')}, ratio={imb.get('ratio')}\n"
        f"Missing: {miss.get('null_pct', 0)}%, mode='{miss.get('mode_value')}'\n"
        f"Data quality: {dq.get('recommended_action')}, "
        f"case issues: {dq.get('case_inconsistency_groups', 0)}, "
        f"whitespace: {dq.get('whitespace_issue_count', 0)}"
        f"{seg_note}"
    )
    ...
    for col in cols:
        ...
        if match:
            ...
            if text:
                result[col] = text
                continue
        log.warning("CategoricalAgent: no LLM insight parsed for column '%s', using stats fallback", col)
...
class CategoricalAgent:
    ...
    def _batch_insights(self, summaries: dict[str, str]) -> dict[str, str]:
        ...
        prompt_parts = [
            "Analyse the following categorical financial data columns. For each column, write 2-3 sentences "
            "of clear, actionable insights focusing on what is unusual, risky, or noteworthy.\n\n"
        ...
        try:
            ...
            response = groq_invoke(batch_llm, msgs, agent_name="categorical")
            content = response.content if isinstance(response.content, str) else ""
            return _parse_batch_insights(content, cols, summaries)
        except Exception as exc:
            log.error("CategoricalAgent batch LLM error: %s", exc)
    ...
    def _run_table(self, df: pd.DataFrame, table: str, routes: list[ColumnRoute]) -> list[AgentResult]:
        ...
        for route in routes:
            col = route["column"]
            try:
                analyses[col] = run_analysis_from_df(df, col, "categorical", self.config)
            except Exception as exc:
                log.error("CategoricalAgent compute error on %s.%s: %s", table, col, exc)
                analyses[col] = {}
        ...
        for i in range(0, len(col_list), _BATCH_SIZE):
            ...
            log.info("CategoricalAgent %s: batch %d-%d insights done", table, i + 1, i + len(batch_cols))
        ...
        for route in routes:
            ...
            try:
                ...
                chart_type, chart_data = self._chart_agent.pick_chart(col, series, df)
            except Exception as exc:
